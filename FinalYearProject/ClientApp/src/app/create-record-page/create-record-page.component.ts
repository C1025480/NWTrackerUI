...
export class CreateRecordPageComponent implements OnInit {
  ...
  saveChanges() {
    ...
    this.http.post<TT_TRACKER>(`https://localhost:7199/UpdateTrackerRecord`, this.Tracker)
      .subscribe(
        result => {
          console.log("API Response: Updated Successfully");
        },
        error => {
          console.error("Error fetching tracker record:", error);
        }
    ...
    console.log('Saving changes:', this.Tracker);
  }
...
interface TT_TRACKER {
  ...
  tT_ACCESS_EQUIPMENT_REQUIRED: boolean;
